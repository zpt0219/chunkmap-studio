...
当前 Inspector Seam tab 只保留邻居方向摘要与 `Edit Seam...` 入口。编辑器使用独立的
modeless ImGui tool window，而不是阻塞 Map Canvas 的 modal；用户拖动时可以同时观察主地图。

```text
Seam Editor — (0,1) Right (1,1)

[Composite] [Difference] [First] [Second]   Feather [24] px
...
Double-click: Add point   Drag: Move   Right-click: Remove

[Reset to Straight]                         [Cancel] [Save Seam]
```

交互规则：

- Composite 是默认模式，直接显示当前融合结果；
- Difference 显示 placement 后两侧原始 overlap 的 RGB difference；
- First/Second 用于临时查看单侧采样；
- 左键拖动 control point；
- 双击折线增加 point；
- 右键中间 point 删除；首尾 point 不可删除；
- point 的 `along` 不能越过相邻 point，`across` 限制在 overlap；
- Feather slider 与 point 拖动均实时重算当前 overlap；
- Preview 同时更新 Seam Editor 和主 Map Canvas；
- Cancel 恢复打开窗口前的参数；
- Save Seam 通过正式 command 保存 JSON，不写 PNG。

第一版不增加自动内容识别折线。Reset 只生成中央直线；用户负责调整路径。后续若增加
Auto Fit，它也只能提出 control points，不能静默保存。
...
- 返回 `changed_chunks=[coord]` 与受影响 Seam keys；
- 不返回处理后正式 PNG。
...
5. **Seam Editor**：modeless window、控制点编辑、实时 preview、Save/Reset command；
...
3. 用户可以在独立 Seam Editor 拖动折线与羽化宽度，并实时看到 editor 和主地图结果。
