...
- Map projects live under `output/<project-name>/` and are intentionally ignored
  by Git.
- Projects persist only `project.json`, `concept.png`, non-empty prompts, Ready
  source PNGs, sparse `placements.json`, and edited Seam JSON. Context handoff
  belongs under `.chunkmap/handoff/`; no derived image cache belongs in a project.
...
- Additional imports and generated writes require exact Chunk dimensions. Their
  formal PNG bytes are preserved; alignment and seam editing must never rewrite them.
  Imports are not silently translated. Generated writes automatically search a
  conservative whole-image translation and persist the result as placement data.
...
3. Read the returned `manifest.json`, `prompt.txt`, `template.png`, and `mask.png`.
   `template.png` contains placed neighbor overlap pixels and a transparent area
   to create. `mask.png` uses white for generation and protects only the outermost
   1px anchor edge where a Ready neighbor exists.
...
4. Use the image-generation capability available through the user's Codex
   ...
   for an API integration. Preserve the black 1px neighbor anchors exactly and
...
The write preserves the generated PNG and performs deterministic automatic
registration, then stores the selected offset as non-destructive placement data and
reports diagnostics in JSON. It does not generate a second image. For imported or existing Ready images,
Desktop Alignment controls offer manual Horizontal/Vertical preview and Auto
suggestion; `Save Placement` persists only the offset and never writes the formal
PNG. Auto compares independent low-resolution 2D and row/column
projection candidates with one shared validation score; the reviewer can preview
either candidate without changing project state.
